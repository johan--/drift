/**
 * Spring Dependency Injection Patterns Semantic Detector
 * 
 * Language-agnostic detector that finds Spring DI patterns
 * by looking for semantic concepts like @Autowired, @Bean, @Qualifier, etc.
 * 
...
export class SpringDISemanticDetector extends SemanticDetector {
  ...
  readonly description = 'Learns dependency injection patterns from Spring codebases';
  ...
  constructor() {
    super({
      ...
      includeStrings: true, // Include strings for SpEL expressions
    });
  }
  ...
  protected createPatternViolation(
    ...
    return {
      ...
      message: `DI pattern differs from codebase norm: using '${match.contextType}' but project primarily uses '${dominantPattern.contextType}'`,
      expected: dominantPattern.contextType,
      actual: match.contextType,
      explanation: `Your codebase uses '${dominantPattern.contextType}' for dependency injection in ${dominantPattern.percentage.toFixed(0)}% of cases ` +
